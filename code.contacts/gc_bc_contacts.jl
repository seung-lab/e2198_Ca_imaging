...
function gc_bc_contacts(blocksize = [64; 64])
			...
			for y = 2:jmax-1
				for x = 2:imax-1
					...
					if cur != blockdata[x-1, y, z] || cur != blockdata[x, y-1, z] || cur != blockdata[x, y, z-1] ||
						...
						# is surface point
						bctype = get(bcDict, cur, "");
						if isempty(bctype)   # non-BC
							#do nothing
						else  # BC
							bcKey = UInt64(cur) << 32;

							cells = Array{UInt64}([blockdata[x-1, y, z],  blockdata[x, y-1, z],  blockdata[x, y, z-1], 
								 blockdata[x+1, y, z],  blockdata[x, y+1, z],  blockdata[x, y, z+1]]);
							keys = cells + bcKey;

							if z==2 || z==soma_high_cut_off-soma_low_cut_off
								coord = [[x, y] + grid_offset, z + soma_low_cut_off];
								warn("boundary: $z $cur")
								push!(exceptions, [cur, coord]);
							end
							for key in keys
								if key != bcKey && key != bcKey+cur
							#display([key >> 32, key << 32 >> 32,])
...
confusion_mat_bctype = [bcDict[_] for _ in confusion_mat[1,:]];
display(typeof(confusion_mat))

#file = matopen("gc_bc_contacts.mat", "w")
matwrite("gc_bc_contacts.mat", Dict(
	"confusion_mat" => confusion_mat,
	"confusion_mat_bctype" => confusion_mat_bctype,
	));
save("gc_bc_contacts.jld", Dict("confusion_mat" => confusion_mat,
	"confusion_mat_bctype" => confusion_mat_bctype,
