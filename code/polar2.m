...
%         error ( 'R must be a 2 element vector' );
    ...
    [theta,rho,radial_limits,line_style,nogrid] = deal(args{1:5});        
end
...
% make a radial grid
    ...
    if ( isempty(radial_limits) )
        ...
        rmax = radial_limits(3);
        rmin = radial_limits(1);
        rstep = radial_limits(2);
...
% draw radial circles
    ...
    if(nogrid~=1)
        for i=(rmin+rinc):rinc:rmax
            hhh = line(xunit*(i-rmin),yunit*(i-rmin),'linestyle',ls,'color',tc,'linewidth',0.5,...
                'handlevisibility','off','parent',cax);
            %         text((i-rmin+rinc/20)*c82,(i-rmin+rinc/20)*s82, ...
            %             ['  ' num2str(i)],'verticalalignment','bottom',...
            %             'handlevisibility','off','parent',cax,'FontSize',18)
        end
    else
        i=rmax;
        hhh = line(xunit*(i-rmin),yunit*(i-rmin),'linestyle',ls,'color',tc,'linewidth',0.5,...
...
% plot spokes
    ...
    sn = [-snt; snt];
    if(nogrid~=1)
    line((rmax-rmin)*cs,(rmax-rmin)*sn,'linestyle',ls,'color',tc,'linewidth',0.5,...
         'handlevisibility','off','parent',cax)
    end
...
if ischar(line_style)
    q = plot(xx,yy,'parent',cax,'linewidth',1);
else
    q = plot(xx,yy,'Color',line_style,'parent',cax,'linewidth',1);
end
